...
// Maintenance Patch with the confirmation email from ralinktech.com
				2012-07-06		modified from RT3070 code base
*/
...
// TODO: temp removed
        ...
        if (pAd->PortCfg.bFastRoaming)
        {
            ...
            DBGPRINT(RT_DEBUG_TRACE, "FastRoaming: RxSignal %d, dBmToRaom %d\n", RxSignal, pAd->PortCfg.dBmToRoam);
	...
	IN	ULONG			Now)
{
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++)
	{
		...
		*/
		...
		// Cannot connect to normal WMTS for settlement (due to Ad-hoc Ghost AP is connected)
		DBGPRINT(RT_DEBUG_TRACE, "   BssType = %d, AuthMode = %d\n", pBss->BssType, pBss->AuthMode);		
	    // Quick fix in Firmware 91 for preventing Ad-hoc AP Bus
	    if (pBss->BssType == BSS_ADHOC && pBss->AuthMode == Ndis802_11AuthModeOpen) {
			DBGPRINT(RT_DEBUG_ERROR, "   Suspect GhostAP: BssType = %d, AuthMode = %d\n", pBss->BssType, pBss->AuthMode);
			DBGPRINT(RT_DEBUG_ERROR, "   Mac %x:%x:%x:%x:%x:%x\n",
				pBss->Bssid[0],
				pBss->Bssid[1],
				pBss->Bssid[2],
				pBss->Bssid[3],
				pBss->Bssid[4],
				pBss->Bssid[5]);
			continue;   // skip if Roaming to ADHOC car
		}
		// End
		// AP passing all above rules is put into roaming candidate table        
		NdisMoveMemory(&pRoamTab->BssEntry[pRoamTab->BssNr], pBss, sizeof(BSS_ENTRY));
	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE)	
{
	...
	if (LengthVIE != 0)
	{
		if (LengthVIE > 255) {
			...
			test2 = pVIE->Length;
			if (test2 > test1) {
				DBGPRINT(RT_DEBUG_ERROR, "*** WARN! mlme.c(%d) LengthVIE=%d, pVIE->Length=%d ***\n", __LINE__, LengthVIE, pVIE->Length);
			}
		}
	...
	IN OUT BSS_TABLE *OutTab) 
{
	...
	{
		if (OutTab->BssEntry[i].Bssid[0] == pAd->PortCfg.oclBssid[0] &&
			OutTab->BssEntry[i].Bssid[1] == pAd->PortCfg.oclBssid[1] &&
			OutTab->BssEntry[i].Bssid[2] == pAd->PortCfg.oclBssid[2] &&
			OutTab->BssEntry[i].Bssid[3] == pAd->PortCfg.oclBssid[3] &&
			OutTab->BssEntry[i].Bssid[4] == pAd->PortCfg.oclBssid[4] &&
			OutTab->BssEntry[i].Bssid[5] == pAd->PortCfg.oclBssid[5]) {
			tmpRssi = OutTab->BssEntry[i].Rssi;	// Record the RSSI, roll back later
			OutTab->BssEntry[i].Rssi = 0x80;	// Firmware RSSI max value
			adjusted = 1;
			DBGPRINT(RT_DEBUG_TRACE, "    SAME MAC address Adjusted!!!\n");
		}
	}
	...
	DBGPRINT(RT_DEBUG_TRACE, "    ********************************\n");
	DBGPRINT(RT_DEBUG_TRACE, "    Start %d\n", OutTab->BssNr);
	for (i=0; i<OutTab->BssNr; i++) {
		DBGPRINT(RT_DEBUG_TRACE, "    Mac[%d] %x:%x:%x:%x:%x:%x = %d[%d dBm]\n", 
			i,
			OutTab->BssEntry[i].Bssid[0],
			OutTab->BssEntry[i].Bssid[1],
			OutTab->BssEntry[i].Bssid[2],
			OutTab->BssEntry[i].Bssid[3],
			OutTab->BssEntry[i].Bssid[4],
			OutTab->BssEntry[i].Bssid[5],
			OutTab->BssEntry[i].Rssi,
			OutTab->BssEntry[i].Rssi - 0x79);
	}
	DBGPRINT(RT_DEBUG_TRACE, "    End\n");
	DBGPRINT(RT_DEBUG_TRACE, "    ********************************\n\n");
	DBGPRINT(RT_DEBUG_TRACE, "    Mac[default] %x:%x:%x:%x:%x:%x\n", 
			pAd->PortCfg.oclBssid[0],
			pAd->PortCfg.oclBssid[1],
			pAd->PortCfg.oclBssid[2],
			pAd->PortCfg.oclBssid[3],
			pAd->PortCfg.oclBssid[4],
			pAd->PortCfg.oclBssid[5]);

	if (adjusted == 1) {
		OutTab->BssEntry[0].Rssi = tmpRssi;
		printk(KERN_INFO "	Adjusted RSSI = %d\n", OutTab->BssEntry[0].Rssi);
		DBGPRINT(RT_DEBUG_TRACE, "    Mac[0] %x:%x:%x:%x:%x:%x = %d[%d dBm]", 
			OutTab->BssEntry[0].Bssid[0],
			OutTab->BssEntry[0].Bssid[1],
			OutTab->BssEntry[0].Bssid[2],
			OutTab->BssEntry[0].Bssid[3],
			OutTab->BssEntry[0].Bssid[4],
			OutTab->BssEntry[0].Bssid[5],
			OutTab->BssEntry[0].Rssi,
			OutTab->BssEntry[0].Rssi - 0x79);
	}
}
	...
	IN OUT	PBSS_ENTRY	pBss)
{
	...
	INT                             OffsetLength;		// Mantis #7223


	OffsetLength=0;	// Mantis #7223
	...
	while (Length > 0)
	{
		...
		switch (pEid->Eid)
		{
			case IE_WPA:
				...
				// To ensure: Length -= (pEid->Len + 2); can be process peacefully
				if (Length < (pEid->Len + 2)) {
DBGPRINT(RT_DEBUG_ERROR, "    (IE_WPA): Length=%d < pEid->Len=%d (+2)\n", Length, pEid->Len);
					break;
				}
				else if ( pEid->Len < 16 ) {
DBGPRINT(RT_DEBUG_ERROR, "    (IE_WPA): pEid->Len=%d (<16)\n", pEid->Len);
					break;
				}
				...
				// Count value check to avoid pTmp out of range				
				if ( (Count*4) > (pEid->Len-16) ){
DBGPRINT(RT_DEBUG_ERROR, "    (IE_WPA): Count=%d < ((pEid->Len=%d -16)/4) \n", Count, pEid->Len);
					break;
				}
...
// 
if (Count > 1) {
DBGPRINT(RT_DEBUG_WARN, "    BssCipherParse.AKM.Count=%d\n", Count);
if (pBss->Ssid != NULL) {
DBGPRINT(RT_DEBUG_WARN, "    BssCipherParse.AKM(%d)Ssid=%s\n", pBss->SsidLen, pBss->Ssid);
}
}
					// Patch this according to driver RT3070
					// www.ralinktech.com
//					while(Count >0)
//					{			    
						pTmp   += 3;
						...
						if (pTmp == NULL) {
DBGPRINT(RT_DEBUG_ERROR, "    ********** HIT NULL *******\n");
							break;
						}
...
//					pBss->AuthModeAux = Ndis802_11AuthModeWPANone;
			...
			case IE_RSN:
				...
				OffsetLength += sizeof(RSN_IE_HEADER_STRUCT);     // Mantis #7223
				...
				OffsetLength += sizeof(CIPHER_SUITE_STRUCT);     // Mantis #7223
...
#endif
				pTmp   += sizeof(USHORT);
				OffsetLength += sizeof(USHORT);     // 
				
				if ( (Count*4) > (pEid->Len-16) ){
DBGPRINT(RT_DEBUG_ERROR, "    (IE_RSN): Count=%d < ((pEid->Len=%d -16)/4) \n", Count, pEid->Len);
                                }

				// Begin
				if ( (Count*sizeof(PCIPHER_SUITE_STRUCT)) > (pEid->Len - OffsetLength) )
				{
					DBGPRINT(RT_DEBUG_ERROR, "    (IE_RSN): Count=%d, (Size=%d) < (pEid->Len - offset=%d), offset=%d \n", Count, Count*sizeof(PCIPHER_SUITE_STRUCT), pEid->Len - OffsetLength, OffsetLength);
				    if (pBss->Ssid != NULL)
					{
						DBGPRINT(RT_DEBUG_ERROR, "    (IE_RSN): BssCipherParse.PairwiseCnt(%d)Ssid=%s\n", pBss->SsidLen, pBss->Ssid);
					}
					break;
				}
				// End
				...
				while (Count > 0)
				{
					...
					pTmp += sizeof(CIPHER_SUITE_STRUCT);
					OffsetLength += sizeof(CIPHER_SUITE_STRUCT);     // Mantis #7223: MIP's DDU sometimes cannot detect BCP
					Count--;
				}
				...
				// 5. Get AKM ciphers
				// 
				if ( (Count*sizeof(AKM_SUITE_STRUCT)) > (pEid->Len - OffsetLength) )
				{
					DBGPRINT(RT_DEBUG_ERROR, "    (IE_RSN): Count=%d, (Size=%d) < (pEid->Len - offset=%d), offset=%d \n", Count, Count*sizeof(AKM_SUITE_STRUCT), pEid->Len - OffsetLength, OffsetLength);
				    if (pBss->Ssid != NULL)
					{
						DBGPRINT(RT_DEBUG_ERROR, "    (IE_RSN): BssCipherParse.PairwiseCnt(%d)Ssid=%s\n", pBss->SsidLen, pBss->Ssid);
					}
				...
				//{
					if (Count > 1)
					{
						DBGPRINT(RT_DEBUG_ERROR, "    : BssCipherParse.AKM.Count=%d..\n", Count);
					}
					pAKM = (PAKM_SUITE_STRUCT) pTmp;

					if (!RTMPEqualMemory(pTmp, RSN_OUI, 3))
					{
						DBGPRINT(RT_DEBUG_ERROR, "    : BssCipherParse AKM break condition fulfilled.\n");
						if (pBss->Ssid != NULL)
						{
							DBGPRINT(RT_DEBUG_ERROR, "    : BssCipherParse.AKM(%d)Ssid=%s\n", pBss->SsidLen, pBss->Ssid);
						}
						break;
					}
