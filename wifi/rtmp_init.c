		...
		{		 
			...
			{		   
				...
				{					
					...
					// 
					if(RTMPGetKeyParameter("OclBssid", tmpbuf, 32, buffer))
					{
						INT 	i;
						PUCHAR	macptr;
					    ...
					    for (i = 0, macptr = rstrtok(tmpbuf,":"); macptr; macptr = rstrtok(NULL,":"), i++)
					    {
				            ...
				            pAd->PortCfg.oclBssid[i] = (UCHAR)simple_strtol(macptr, 0, 10);

				            DBGPRINT(RT_DEBUG_TRACE, "%s:::(oclBssid[%d]=%x)\n", __FUNCTION__, i, pAd->PortCfg.oclBssid[i]);
				            printk(KERN_INFO "%s:::(oclBssid[%d]=%x)\n", __FUNCTION__, i, pAd->PortCfg.oclBssid[i]);
					    }
					}
